...
public class Graph {
    ...
    public boolean getRoute(Vertex start, Vertex end){
        
    	Queue<Vertex> route_queue = new LinkedList<>();
    	Set<Vertex> visited_vertices = new HashSet<>();
    	
    	visited_vertices.add(start);
    	route_queue.add(start);
        
    	while(!route_queue.isEmpty()) {
    		Vertex current = route_queue.remove();
    		...
    		for(Edge e : neighbors) {
    			if(visited_vertices.contains(e.getOne()) && visited_vertices.contains(e.getTwo()))
    				continue;
    			else {
    				if(!visited_vertices.contains(e.getOne())) {
    					route_queue.add(e.getOne());
    					visited_vertices.add(e.getOne());
    					System.out.println(e.getOne().label);
    				}
    				
    				if(!visited_vertices.contains(e.getTwo())) {
    					route_queue.add(e.getTwo());
    					visited_vertices.add(e.getTwo());
    					System.out.println(e.getTwo().label);
    				}
    			}
    		}
    	}
